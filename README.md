...
> Archive-Manager - Open source tool by AetherCodeHQ

`archive` `cli` `compression` `golang` `open-source` `utilities`

## What is it?

**Archive Manager** is a cli tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
```bash
# Basic usage
./Archive-Manager --path ./target

# With options
./Archive-Manager --path ./target --format json --output report.json

# Verbose mode
./Archive-Manager --path ./target --verbose
```
...
### Basic scan
```bash
./Archive-Manager --path ./my-project
```

### JSON report
```bash
./Archive-Manager --path ./my-project --format json --output report.json
```
...
- name: Run Archive Manager
  run: ./Archive-Manager --path . --format json --output report.json
```
